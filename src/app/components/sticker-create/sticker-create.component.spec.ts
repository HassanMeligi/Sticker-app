...
removeStickerImage(index: number) {
  ...
  onStickerImageChange(index: number, event: any) {
    const file = event.target.files[0];
    if (file) {
      this.stickers.at(index).patchValue({ imageFile: file });
      const reader = new FileReader();
      reader.onload = () => this.stickerImagePreviews[index] = reader.result as string;
      reader.readAsDataURL(file);
    }
  }
  ...
  submit() {
    ...
    console.log('Form Validity:', this.stickerForm.valid);
    console.log('Form Data:', this.stickerForm.value);

    if (this.stickerForm.valid) {
      const formValue = { ...this.stickerForm.value };
      formValue.stickers = formValue.stickers.map((s: any) => ({
        ...s,
        emojis: typeof s.emojis === 'string' ? s.emojis.split(',').map((e: string) => e.trim()) : s.emojis
      }));

      const formData = this.stickerService.buildFormData(formValue);
      this.stickerService.createStickerPack(formData).subscribe({
        next: () => this.snackBar.open('Sticker Pack Created!', 'Close', { duration: 3000 }),
        error: (err: any) => {  // ✅ Explicit 'err' type
          console.error('Error:', err);
          this.snackBar.open('Error creating sticker pack.', 'Close', { duration: 3000 });
        }
      });
    } else {
      console.warn('Form invalid');
      this.snackBar.open('Form is invalid. Please fill all required fields.', 'Close', { duration: 3000 });
    }
  }
